...
xcrun simctl spawn $UDID defaults write com.shera.promptnote iCloud_disabled -bool YES
xcrun simctl launch $UDID com.shera.promptnote
```
...
`xcrun simctl spawn $UDID defaults read com.shera.promptnote last_seen_whats_new_version`
...
```sh
plutil -p "$(xcrun simctl get_app_container $UDID com.shera.promptnote data)/Library/Preferences/com.shera.promptnote.plist"
```
...
- **Persistence**: autosave writes the note into the app container —
  `ls "$(xcrun simctl get_app_container $UDID com.shera.promptnote data)/Documents/InboxFolder/"`
...
container (`xcrun simctl get_app_container $UDID com.shera.promptnote data`) appears in
...
```sh
xcrun simctl openurl $UDID "file://$(xcrun simctl get_app_container $UDID com.shera.promptnote data)/Documents/InboxFolder/<note>.pop"
```
...
- **idb cannot tap the tool picker's ⋯ popover**: `describe-all` lists its rows (Auto-Minimize,
  ...
  `os.Logger` category under the `com.shera.promptnote` subsystem. Prove the sweep against a string
